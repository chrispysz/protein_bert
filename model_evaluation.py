...
def predict_window(sequences, model, seq_cutoff=SEQ_CUTOFF, batch_size=32):
...
def predict_window_40(sequences, model, seq_cutoff=SEQ_CUTOFF, batch_size=32):
    """Predicts model output for a window in the sequence and return list of predictions."""
    all_predictions = []
    for i in tqdm(range(0, len(sequences), batch_size)):
        batch_sequences = sequences[i:i+batch_size]
        ...
        # Flattening the predictions and extending all_predictions list
        flattened_preds = [pred[0] for pred in preds]
        all_predictions.extend(flattened_preds)
...
def main(dataset_path_neg, dataset_path_pos, predictions_cache, model_path):
    ...
    if dataset_path_neg in predictions_cache:
        ...
        if (dataset_path_neg == 'PB40.csv'):
            y_scores_neg = predict_window_40(sequences_neg, model)
        else:
            y_scores_neg = predict_window(sequences_neg, model)
    ...
    if dataset_path_pos in predictions_cache:
        ...
        y_scores_pos = predict_window(sequences_pos, model)
    ...
    plt.savefig('./histograms/' + model_path + dataset_path_pos_clean + '_histogram.png')

    plt.figure()
    plot_histogram(y_scores_neg, dataset_path_neg_clean)
    plt.savefig('./histograms/' + model_path + dataset_path_neg_clean + '_histogram.png')
    ...
    with open(f'./csvs/{dataset_path_pos_clean}_{dataset_path_neg_clean}.csv', 'a+', newline='') as csvfile:
        ...
        row = ["bass_" + model_path, len(labels_pos), len(labels_neg), auroc, avg_prec] + list(recall_at_fprs)
...
if __name__ == "__main__":
    ...
    for model_path in model_paths:
        ...
        for neg, pos in pairs:
            main(neg, pos, predictions_cache, model_path)
